...
| V2 | **LIVRÉ** : catalogue `SupervisionAutomatableTypes` avec **niveau max par type** (CLEANING/REVIEW_DRAFT → FULL ; PRICE_DROP/CALENDAR_BLOCK/cautions → NOTIFY — jamais silencieux) intégré au gate (étape 2 bis, min(règle, plafond module, max type)) · CALENDAR_BLOCK N1 (maxAutoBlockDays 7 + cap 1 auto/bien/7j via appliedBy) · cautions N1 (« aucune Issue ouverte » OPEN/QUALIFIED sur la propriété — lien Issue↔résa NON retenu, filtre propriété conservateur ; délai post-checkout recalculé ; annulation/statut caution re-lus ; releaseHold uniquement, MONEY_TOOLS intact). +21 tests. | ✅ |
| V3 | **LIVRÉ** : PAYMENT_REMINDER démasqué max NOTIFY (enveloppe : aucune relance APPLIED pour la résa + anti-rafale 72 h toute carte ; 2ᵉ relance toujours HITL ; `reservationId` porté par les cartes) · migration 0342 (`suggested_at`/`suggestion_dismissed_at`) · **Règles de Confiance des cartes** : `SupervisionCardTrustService` + scheduler quotidien (seuil 5 applies humains consécutifs, DISMISSED remet à zéro, `auto:gate` neutre, types catalogue only, cooldown 30 j) → suggestion INERTE + notif `SUPERVISION_AUTO_RULE_SUGGESTED` → chip « Recommandé » + Activer/Ignorer dans l'UI. +19 tests. | ✅ |

**CHANTIER TERMINÉ (2026-07-11)** — vérifié à chaque vague (mvn complet + tsc = 0). V1 committée (b5f840d8) ; V2+V3 à committer. Migrations 0341+0342 = flux Liquibase normal au boot ; tout opt-in, zéro changement de comportement sans activation.
