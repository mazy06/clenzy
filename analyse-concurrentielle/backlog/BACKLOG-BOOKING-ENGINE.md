...
| 2.16 | **Studio UX** : colonnes réductibles, marges, analyse du design (réintégration) | 🔄 | ⭐⭐ | S | — | 🔄 **fait** : colonnes BlocsTree + pane Bloc/Thème/CSS **réductibles** (icônes seules, `PanelLeft/Right`), marges topbar/toolbar réduites ; **« Analyse du design »** réintégrée dans la **palette ⌘K** + **icône topbar** → modale (réutilise `AiDesignMatcher` : URL → analyse IA → tokens) appliquée en direct au widget + blocs (`cfg.patch`). **Reste** : afficher le **site cible** (snapshot `SiteSnapshotService`) + **y placer le widget** (preview embed cross-frame). |
