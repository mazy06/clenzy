...
| 2.3 | + de blocs (Galerie/lightbox, Carte, FAQ, Formulaire, Table prix, Stats, Logos, Vidéo, Détail propriété) | 🔄 | ⭐⭐ | M | — | 🔄 **4 blocs ajoutés** : **FAQ** (Q\|R), **Galerie** (grille d'images URL + colonnes), **Chiffres clés** (Valeur\|Libellé), **Vidéo** (embed YouTube/Vimeo). Registre + `blockStyles.css` + sélecteurs CSS. **Reste** : Carte, Table de prix, Logos, Formulaire, Détail propriété, lightbox + miroir SSR `clenzy-sites/blocks.tsx`. |
