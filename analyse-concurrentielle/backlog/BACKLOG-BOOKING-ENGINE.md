...
|---|------|------|--------|--------|--------|
| 1.1 | Service SSR **Next.js « Clenzy Sites »** + modèles `Site`/`SitePage`/`SiteDomain` + domaines custom + TLS (Cloudflare for SaaS) | 🔄 | ⭐⭐⭐ | XL | infra clenzy-infra | 🔄 **socle backend data FAIT** — entités `Site`/`SitePage`/`SiteDomain` + enums (`SiteStatus`/`SitePageType`/`SiteDomainStatus`) + repos + migration 0254 (`sites`/`site_pages`/`site_domains`). `Site` = nouvelle couche, widget (`BookingEngineConfig`) séparé + référençable. `mvn package` vert. **Reste** : (a) CRUD admin sites/pages/domaines, (b) **API de livraison publique** (résolution hostname→site→pages+SEO) pour le SSR, (c) **service Next.js « Clenzy Sites »** (nouveau repo/app), (d) **Cloudflare for SaaS** (custom hostnames + TLS) + déploiement clenzy-infra = **infra, implication utilisateur**. |
