...
| 2.2 | Multi-page (`SitePage[]` + nav + routing) | 🔄 | ⭐⭐ | L | 1.1 | 🔄 **Cœur livré** : backend `POST /api/sites/ensure-for-config/{configId}` (find-or-create du site + migration `pageLayout`→page d'accueil). Front : `sitesApi` + hook `useSitePages` + `PagesBar` (onglets : ajouter/renommer/supprimer, accueil protégée) + `DesignBuilder` édite/persiste **par page** (auto-save au changement de page ; miroir accueil→`config.pageLayout` pour la page publique React ; repli mono-page si API sites KO). **Reste** : réordonner les pages, SEO/path par page dans l'inspecteur, routing/nav SSR `clenzy-sites` + miroir blocs 2.3/2.4. |
