...
| 2.12 | Capture leads (exit-intent/pré-checkout/form embarquable) + abandoned multi-étapes + segments Brevo | 🔄 | ⭐⭐ | M | — | 🔄 **Gros avancement** : **(1) form embarquable exit-intent** dans le widget (`LeadCapture.ts` : modal exit-intent 1×/session, consentement RGPD, `POST /leads` source EXIT_INTENT, repli silencieux si 403/désactivé ; i18n fr/en/ar ; flag d'init `leadCapture`). **(2) flag `leadCaptureEnabled`** exposé dans le DTO config public (org-level). **(3) relance panier abandonné MULTI-ÉTAPES** : `reminder_count` (migration 0256) + `recordReminderSent` + scheduler échelonné **1h / 24h / 72h** avec sujets/corps escaladés. *Reste : trigger pré-checkout + **segments/listes Brevo** (API listes Brevo — la relance/lead n'alimente pas encore de segments).* |
