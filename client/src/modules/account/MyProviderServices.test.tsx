...
it('edits a free service containing a slash through the canonical endpoint',async()=>{
  ...
  api.get.mockResolvedValue({services:[service],options:[]});api.put.mockResolvedValue({...service,amount:30});
...
it('requires the billing unit before saving a per-unit price',async()=>{
  api.get.mockResolvedValue({services:[],options:[{key:'cleaning',labelFr:'Ménage',labelEn:'Cleaning'}]});
