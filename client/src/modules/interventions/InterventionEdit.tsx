...
export default function InterventionEdit() {
  ...
  // Si l'utilisateur n'a pas la permission de modifier des interventions, afficher un message informatif
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Charger l'intervention existante
        if (id) {
          const interventionResponse = await fetch(`${API_CONFIG.BASE_URL}/api/interventions/${id}`, {
            headers: {
          ...
          if (interventionResponse.ok) {
            ...
            console.log('🔍 InterventionEdit - Intervention chargée:', interventionData);
            ...
            setFormData({
              ...
              scheduledDate: interventionData.scheduledDate ? new Date(interventionData.scheduledDate).toISOString().slice(0, 16) : '',
              estimatedDurationHours: interventionData.estimatedDurationHours || 1,
              actualDurationMinutes: interventionData.actualDurationMinutes || undefined,
              estimatedCost: interventionData.estimatedCost || undefined,
              actualCost: interventionData.actualCost || undefined,
        ...
        console.error('🔍 InterventionEdit - Erreur chargement:', err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = id 
        ? `${API_CONFIG.BASE_URL}/api/interventions/${id}`
        : `${API_CONFIG.BASE_URL}/api/interventions`;
      ...
      if (response.ok) {
        ...
        console.log('🔍 InterventionEdit - Intervention sauvegardée:', savedIntervention);
      ...
      console.error('🔍 InterventionEdit - Erreur sauvegarde:', err);
    ...
    return (
      <Box>
        ...
        <Alert severity="error">
          Vous n'avez pas les droits pour modifier cette intervention.
        </Alert>
      </Box>
    );
  }
