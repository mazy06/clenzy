-- 0066: Amélioration du système d'auto-assignation
-- - Colonnes retry/status sur service_requests (avec DEFAULT)
-- - Table audit des assignations
-- - Table paramètres workflow par organisation

-- 1) Colonnes retry sur service_requests
-- Les colonnes ont pu être créées par Hibernate DDL-auto sans DEFAULT.
-- On s'assure qu'elles existent avec un DEFAULT correct.
...
-- Corriger les lignes existantes où auto_assign_retry_count est NULL
UPDATE service_requests SET auto_assign_retry_count = 0 WHERE auto_assign_retry_count IS NULL;

-- Ajouter le DEFAULT pour les futures insertions
ALTER TABLE service_requests ALTER COLUMN auto_assign_retry_count SET DEFAULT 0;
