-- Réaligne les contraintes CHECK de notifications sur les enums Java (NotificationKey/NotificationCategory).
-- La dernière liste (0099) anterieure a ~18 clés (CHANNEX_*, IOT_*, INCIDENT_*, BRIEFING_READY, RESERVATION_*, …)
-- dont les inserts échouaient silencieusement (try/catch NotificationService). Ajoute aussi les nouvelles
-- clés ACCESS_CODE_ROTATED / GUEST_DOOR_UNLOCKED et la catégorie RESERVATION.
