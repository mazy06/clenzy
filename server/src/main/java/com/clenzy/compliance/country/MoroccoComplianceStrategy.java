...
/**
 * Strategie de conformite pour le Maroc — norme DGI.
 *
 * Exigences :
 * - Numerotation sequentielle (FACTURE, DEVIS)
 * - ICE (Identifiant Commun de l'Entreprise) obligatoire sur les factures
 * - Mentions legales similaires a la France avec specificites marocaines
 * - Paiement a 30 jours date de facture
 */
@Component
public class MoroccoComplianceStrategy implements CountryComplianceStrategy {
    ...
    public Map<String, Object> resolveComplianceTags(DocumentType type, String legalNumber) {
        ...
        if (type == DocumentType.FACTURE) {
            tags.putIfAbsent("conditions_paiement",
                "Paiement a 30 jours date de facture. Penalites de retard applicables conformement a la legislation en vigueur.");
    ...
    public Map<String, List<String>> buildMentionTagMapping() {
        ...
        mapping.put("identite_vendeur", List.of("entreprise.nom", "entreprise.adresse", "entreprise.ice",
            "${entreprise.nom}", "${entreprise.adresse}", "${entreprise.ice}"));
        ...
        mapping.put("ice_vendeur", List.of("entreprise.ice", "${entreprise.ice}"));
