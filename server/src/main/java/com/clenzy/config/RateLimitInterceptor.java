...
public class RateLimitInterceptor implements HandlerInterceptor {
    ...
    // Fix M-NEW-3 : protection brute-force des codes voucher (endpoint public,
    // non authentifie). 20 essais/min/IP suffit largement pour un guest qui
    // tape son code, bloque l'enumeration auto.
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (path.equals("/api/auth/session") || path.equals("/api/permissions/sync")) {
            ...
            // Fix M-NEW-3 : limite stricte par IP sur la validation publique
            // de code voucher (protection brute-force).
