 ...
 * ({@link AiTargetResolver#resolveChain}) : NVIDIA → Anthropic → OpenAI.</p>
 *
...
public class FailoverChatLLMProvider implements ChatLLMProvider {
    ...
    private List<ResolvedTarget> failoverChainSafe() {
        try {
            Long orgId = tenantContext.getOrganizationId();
            if (orgId == null) {
                return List.of();
            }
            return targetResolver.resolveChain(orgId, AiFeature.ASSISTANT_CHAT, null);
        } catch (Exception e) {
