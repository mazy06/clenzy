 ...
 *
 * <p>{@code X-Forwarded-For} n'est honoré que si le pair direct est un proxy de
 * confiance, et la chaîne est parcourue de DROITE à GAUCHE en sautant les proxies
 * de confiance : nginx AJOUTE l'IP réelle en fin de chaîne
 * ({@code proxy_add_x_forwarded_for}), les entrées de gauche sont fournies par le
 * client et donc spoofables. Sans cette garde, le signataire pourrait forger l'IP
 * attestée dans le certificat de signature via un simple en-tête (Z4B-SECBUGS-01).</p>
 *
 * <p>Logique alignée sur {@code RateLimitInterceptor.getClientIp()} (package
 * config, méthodes package-private — non réutilisables d'ici). Une unification
 * dans {@code com.clenzy.util} est un refactoring candidat (Rule of Three).</p>
 */
public final class TrustedClientIpResolver {
    ...
    /** Visibilité package-private pour les tests. */
